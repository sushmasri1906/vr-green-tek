...
function ServiceCard({ s, tab }: { s: Service; tab: Category }) {
	const Icon = s.Icon;
	return (
		<div
			className={[
				"group relative flex min-h-77.5 flex-col overflow-hidden rounded-3xl",
...
export default function ServicesWeOffer() {
	...
	const settings = {
		...
		responsive: [
			...
			// ✅ keep slick only for >= 640 (mobile uses native scroll)
			{ breakpoint: 640, settings: { slidesToShow: 2 } },
		],
	};

	return (
		<section className="relative bg-[#F8FAFC] py-16 sm:py-18">
			<div className="relative mx-auto max-w-7xl px-4 sm:px-6">
				...
				{/* ✅ MOBILE: native horizontal scroll (best) */}
						...
						<div className="flex gap-4 snap-x snap-mandatory">
							{filtered.map((s) => (
								<div
									...
									<ServiceCard s={s} tab={tab} />
								</div>
							))}
						</div>
					</div>
					{/* optional hint */}
					<div className="mt-4 flex justify-center text-xs text-[#111827]/45">
						Swipe →
					</div>
				</div>
